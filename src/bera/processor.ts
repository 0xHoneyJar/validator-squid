...
const BASE_QUEST_ADDRESSES = Object.values(QUESTS_CONFIG[CHAINS.BERACHAIN])
  .flatMap((quest) => quest.steps.map((step) => step.address.toLowerCase()))
  .filter((value, index, self) => self.indexOf(value) === index); // Remove duplicates

export const processor = new EvmBatchProcessor()
  .setGateway(ARCHIVE_GATEWAYS[CHAINS.BERACHAIN])
  .setRpcEndpoint({
    url: assertNotNull(
      RPC_ENDPOINTS[CHAINS.BERACHAIN],
  ...
  .setBlockRange({ from: BLOCK_RANGES[CHAINS.BERACHAIN].from })
  .addLog({
