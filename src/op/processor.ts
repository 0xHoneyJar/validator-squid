...
const OP_QUEST_ADDRESSES = Object.values(QUESTS_CONFIG[CHAINS.OPTIMISM])
  .flatMap((quest) => quest.steps.map((step) => step.address.toLowerCase()))
  .filter((value, index, self) => self.indexOf(value) === index); // Remove duplicates

export const processor = new EvmBatchProcessor()
  .setGateway(ARCHIVE_GATEWAYS[CHAINS.OPTIMISM])
  .setRpcEndpoint({
    url: assertNotNull(RPC_ENDPOINTS[CHAINS.OPTIMISM], "No RPC endpoint supplied"),
    rateLimit: 10,
  })
  ...
  .setFields({
    ...
    .setBlockRange({ from: BLOCK_RANGES[CHAINS.OPTIMISM].from })
  .addLog({
    address: OP_QUEST_ADDRESSES,
    topic0: [
